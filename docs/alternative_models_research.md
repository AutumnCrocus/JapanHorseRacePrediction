...
## 6. Cold Start対策モデル (For New Horses)

新規馬（新馬戦・未勝利戦・初データの馬）の予測精度を高めるためのアプローチです。

- **課題**:
  - 従来のIDベースモデル（Collaborative Filtering / Bayesian Rating）は、過去の出走履歴がない馬に対して無力（Cold Start問題）。
- **解決アプローチ**:
    1. **Content-Based Feature Engineering (GBDT強化)**
        - 馬IDではなく「属性（Content）」で予測する。
        - **血統統計**: 父馬（Sire）・母父（Broodmare Sire）の産駒成績集計（芝/ダート別勝率、距離適性など）。
        - **人脈統計**: 騎手・調教師の過去データ（コース相性、直近好走率）。
        - **属性**: セリ価格、生産牧場ランク、兄弟馬の成績。
    2. **Factorization Machines (FM / DeepFM)**
        - **特徴**: スパースなデータ（IDの組み合わせ）と密な特徴量（タイム、体重）の相互作用（Interaction）を低次元ベクトルでモデル化する。
        - **強み**: 「騎手×コース」「種牡馬×馬場状態」といった組み合わせ効果を、個別の履歴データが少なくても推定できる（類似の組み合わせから推論）。
        - **推奨ツール**: `xLearn`, `DeepCTR-Torch` (DeepFM)。
    3. **Graph Neural Networks (GNN)**
        - **構造**: 馬・騎手・調教師・父・母を「ノード」、関係性（出走、血統）を「エッジ」としたグラフを構築。
        - **推論**: 新規馬ノードも「父ノード」や「母ノード」と接続されるため、GNNのメッセージパッシングにより親の埋め込み表現（特徴）を継承できる。
        - **強み**: 複雑な血統ネットワークや、騎手-調教師のネットワーク効果を直接学習可能。

## 提言（更新版）

- **フェーズ3.5: Cold Start対策の実装**
  - **Step 1 (即効性あり)**: **LightGBMへの「血統・人脈統計特徴量」の追加。**
    - モデルを変更せず、特徴量エンジニアリングのみで対応可能。最も低コストかつ確実。
  - **Step 2 (先進的)**: **Factorization Machines (DeepFM)** の導入。
    - GBDTでは捉えきれない「ID間の相互作用」をモデル化し、Cold Startに強いサブモデルとして構築し、LightGBMとアンサンブルする。

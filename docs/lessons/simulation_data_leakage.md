...
## 4. 推論時と学習時の特徴量乖離 (Training/Inference Serving Skew) (2026-02-20)

* **事象**: LTRモデルの予測順位が馬番順になっていた。
* **原因**: 推論時に「出馬表」のみを参照し、予測に必要な「血統」「過去成績」などの外部データをロードしていなかったため、全頭が同一条件（特徴無し）とみなされていた。
* **教訓**: 保存したモデルを使用するリアルタイム推論では、**学習時と全く同一のデータパイプライン**（外部データの結合プロセスを含む）が機能していることを保証しなければならない。
* **対策**: `app.py` におけるマスターデータのグローバルロードと、推論データへの自動結合を実装。詳細は `docs/lessons/ranker_feature_matching.md` を参照。

...
## 2026-02-13: 予測レポートのオッズ表示不備（同一値・999.9x）

### 1. スクレイピング時における HTML の改行文字 (`\n`) の影響

* **事象**: `yoso/mark_list.html`（予想オッズページ）から単勝オッズを抽出する際、本来取得できるはずのオッズが無視され、デフォルト値や不正確な値が表示されていた。
* **原因**: パース対象の HTML 要素（`dt` タグ）内のテキストに改行文字が含まれており、`if dt.text == "単勝オッズ":` という完全一致判定が失敗していた。具体的には `"単勝オッズ\n"` のようになっていたため、条件を通過できなかった。
* **教訓**: Webスクレイピングにおいて、タグ内のテキストを取得して比較する場合は、**常に `.strip()` で空白・改行を除去するか、部分一致 (`in`) または正規表現を使用する**のが鉄則である。
* **対策**: `dt.text.strip()` を使用して判定するように修正した。

### 2. pandas `FutureWarning` と実行時ノイズ

* **事象**: `predict_tomorrow.py` などのスクリプト実行時に、pandas の `fillna` や `astype` に伴う `FutureWarning` が大量に出力され、本来のエラーや進捗（tqdm）が見づらくなっていた。
* **原因**: pandas の将来的な仕様変更（ダウンキャストの挙動）に関する警告であり、動作自体に支障はないものの、ユーザーに「エラーが発生している」という誤認を与える原因となっていた。
* **教訓**: 本番運用やレポート生成スクリプトでは、**既知の安全な警告は積極的に抑制 (warnings.filterwarnings)** し、実行ログの視認性（Signal-to-Noise Ratio）を高く保つべきである。
* **対策**: `modules/preprocessing.py` の冒頭で警告を抑制し、`pd.set_option('future.no_silent_downcasting', True)` を設定した。

## 2026-02-14: 予算に合わせた三連複BOX頭数の動的調整

### 1. 予算と組合せ数（BOX頭数）の線形性

* **事象**: 予算1000円で三連複BOXを組む際、4頭（4点/400円）から5頭（10点/1000円）へ拡張した。
* **原因**: ユーザーの予算（1000円）を最大限活用しつつ、的中率を向上させるため。
* **教訓**: 三連複BOXは頭数を1増やすだけで点数が大きく跳ね上がるため、予算ギリギリの頭数設定が「回収率」と「的中率」のトレードオフの境界線になる。
  * 4頭BOX: 4点（400円）
  * **5頭BOX: 10点（1000円）★ 今回選定**
  * 6頭BOX: 20点（2000円）

## 2026-02-14: [CODE QUALITY/DEBUG] Flake8対応と辞書キー名の罠

* **事象**:
  * `predict_..._ltr_box5.py` 作成時、Flake8チェックを通さずに実行したため、未使用インポートやフォーマット違反が多発した。
  * `BettingAllocator` の戻り値キー名を `amount` と誤認し、`KeyError` でレポート生成に失敗した（正しくは `total_amount`）。
  * Windows環境での `sed` コマンドによる空白削除が期待通り動作しなかった。

* **原因**:
  * 既存コード (`predict_..._ltr.py`) からのコピペによる実装で、キー名の確認不足が生じた。
  * Flake8の `W293` (blank line contains whitespace) はエディタ設定やGit設定によっては混入しやすい。
* **対策**:
  * 新規スクリプト作成時は、実行前に必ず `flake8` をかける癖をつける。
  * `BettingAllocator` 等の共通モジュールを使用する際は、戻り値の型とキー名をソースコードレベルで確認する。
  * ファイルの一括整形には `sed` ではなく Python スクリプト (`open(..., newline='\n')`) を使用するのが確実。

* **対策**: `BettingAllocator` に頭数別のBOXメソッドを用意し、予算に応じて最適な戦略を切り替えられるように設計した。

## 2026-02-14: スクレイピング時のテーブル取得漏れとテストの重要性

### 1. `pd.read_html` の仕様とテーブル選択の罠

* **事象**: `simulate_ltr_strategies.py` で過去データのシミュレーションを行った際、単勝・馬連戦略の的中数が「0」になる現象が発生した。
* **原因**: `pd.read_html` はページ内の全テーブルをリストで返すが、コードが `if "3連複" in str(df.values): return df` のように、特定のキーワード（3連複）を含む最初のテーブルだけを返して終了していたため、単勝や馬連の情報が含まれる別のテーブルが無視されていた。
* **教訓**:
  * スクレイピング対象のページ構造（複数のテーブルに分かれているか）を正確に把握する。
  * `pd.read_html` の結果をループ処理する際は、**「条件に合致する全てのテーブル」を収集して結合 (`pd.concat`) する**設計にするのが安全である。
  * 特定の単語（"単勝"など）が含まれているかどうかの判定は、`df.to_string()` や `str(df.values)` でデータ全体を文字列化して行うのが手っ取り早いが、全テーブルを網羅できているか確認するログを入れるべきだった。
* **対策**: 取得した全てのデータフレームに対して、払い戻しに関連するキーワード（単勝、複勝、馬連など）が含まれているかを検査し、該当するものを全てリストに格納した上で、最後に `pd.concat` して返すように修正した。

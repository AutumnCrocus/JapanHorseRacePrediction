...
def generate_full_detailed_report():
    ...
    for venue_id, venue_name in venue_map.items():
        ...
        # 開催回(kai)と日目(day)の特定 (debug_race_idsの結果に基づく)
        ...
        for race_num in range(1, 13):
            ...
            try:
                # fetch_and_process_race_data は app.bias_map などが必要なため、appから渡す
                df_processed = fetch_and_process_race_data(
                    race_id, 
                    app.PROCESSORS.get('lgbm'), # 代表してLGBMのを使用
                ...
                # レース情報を取得するために再度モジュールからメタ情報を抜く（Shutubaを直接見る方が確実な場合も）
                from modules.scraping import Shutuba
                # キャッシュのような役割でfetch_and_process_race_dataの引数になっていたはず
                ...
                for model_type, strategy in model_strategies:
                    ...
                    for idx, (_, row) in enumerate(df_processed.iterrows()):
                        # 前処理済みのdfには馬名や馬番がない場合があるため、元のを取得
                        results.append({

...
from modules.constants import MODEL_DIR, RAW_DATA_DIR, HORSE_RESULTS_FILE, PEDS_FILE  # noqa: E402
from modules.scraping import Shutuba, Odds  # noqa: E402
from modules.training import RacePredictor  # noqa: E402
...
def process_race(race_id, predictor, budget=1000, horse_results_db=None, peds_db=None):  # noqa: C901
    """1レース分の予測実行"""
    ...
    # 予算配分 (box5_sanrenpuku)
    recs = BettingAllocator.allocate_budget(
        results_df,
        budget=budget,
        odds_data=odds_data,
        strategy='box5_sanrenpuku'
    )
...
def main():  # noqa: C901
    ...
    # レポート作成
    report_file = "reports/prediction_20260214_ltr_box5.md"
    os.makedirs("reports", exist_ok=True)

    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 2026年2月14日 競馬予想レポート (LTRモデル / BOX5)\n\n")
        f.write(f"生成日時: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("モデル: LambdaMART (Ranking)\n")
        f.write("戦略: box5_sanrenpuku (3連複5頭BOX)\n")
        f.write("予算: 1レース1000円\n\n")

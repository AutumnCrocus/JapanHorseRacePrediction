...
def extract_bets_from_report(report_path):
    bets_by_race = {}
    ...
    for line in lines:
        ...
        # レースヘッダーの抽出
        # 例: ## 東京1R (202605010701)
        m_race = re.match(r'##\s+.*?\((\d+)\)', line)
        if m_race:
            current_race_id = m_race.group(1)
            bets_by_race[current_race_id] = []
            current_model_strategy = None
            continue

        # モデル・戦略ヘッダーの抽出
        # 例: ### モデル: stacking / 戦略: box4_sanrenpuku (自信度: S)
        ...
        # 買い目の抽出
        # 例: - 3連複 BOX (2,1,5,13 BOX): 5000円 (4点)
        # 例: - ワイド 流し (軸:2 - 相手:1,5,13,11): 1600円 (4点)
        # 例: - 単勝 SINGLE (2): 1000円 (1点)
        if line.startswith('- ') and current_model_strategy and current_race_id:
            ...
            if m_bet:
                ...
                # ソートして保存 (馬単・3連単以外は組み合わせの順序を問わない)
...
def verify_bets(bets_by_race, payouts):
    ...
    for race_id, bets in bets_by_race.items():
        if race_id not in payouts:
            # print(f"Warning: No payout data for race {race_id}")
            continue
            
        race_payout = payouts[race_id] # {type_key: {comb: payout, ...}}
        ...
        for bet in bets:
            ...
            if type_key in race_payout:
                ...
                # combが単数 (1,) の場合と複数 (1, 2, 3) の場合で処理を分ける
...
def main():
    ...
    bets_by_race = extract_bets_from_report(REPORT_FILE)
    
    # いくつ抽出できたか表示
    total_races = len(bets_by_race)
    total_bets = sum(len(b) for b in bets_by_race.values())
    print(f"Extracted {total_bets} bets from {total_races} races.")
    
    print(f"Reading Payouts: {PAYOUT_FILE}")
    ...
    # CSV出力
    out_csv = f"reports/verify_result_{TARGET_DATE}.csv"

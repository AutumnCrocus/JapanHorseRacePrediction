...
def load_resources_original():
    ...
    # Random 500 Races as TARGET
    all_race_ids = df_all['race_id'].unique()
    if len(all_race_ids) > 500:
        import random
        random.seed(42)  # Same seed for comparison
        target_race_ids = sorted(random.sample(list(all_race_ids), 500))
        print(f"Randomly selected 500 races from {len(all_race_ids)} for target simulation.", flush=True)
    else:
...
def run_vectorized_simulation():
    ...
    for chunk in tqdm(race_chunks):
        ...
        for race_id, race_df_preds in grouped:
            ...
            # Analyze Bias using FULL daily data (df_all)
            # Filter df_all for this date/venue efficiently
            # Note: df_all is large, filtering every loop might be slow.
            # But we only do it 500 times.
    ...
    # Report
    report_path = 'simulation_report_track_bias.md'
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("# トラックバイアス戦略シミュレーションレポート\n\n")

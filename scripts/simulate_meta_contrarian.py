"""
新戦略 `meta_contrarian` 検証シミュレーション
- formation_flex (既存) vs meta_contrarian (逆張り) の比較
- 10%サンプルで実行
"""
import os
import sys
            ...
            try:
                ...
                if bet_type in ['単勝', '複勝']:
                    ...
                    if method in ['流し', 'NAGASHI', '1軸流し', 'FORMATION'] and formation and len(formation) >= 2:
                        # FORMATION: [[1着候補], [2着候補], [3着候補]] or [[軸], [相手]]
                        if bet_type == '3連複':
                            # [[軸], [相手]]
                            ...
                            # [[1着], [2着], [3着]]
                            # formation_flexやmeta_contrarianの実装に合わせて柔軟に
                            if len(formation) == 3:
...
def run_simulation(strategy_name: str, predictor, hr, peds, df_target, returns_dict, race_ids, logger):
    ...
    for i, race_id in enumerate(race_ids):
        print(f"[{strategy_name}] Processing race {i+1}/{len(race_ids)}: {race_id}", end='\r')
        try:
...
def main():
    ...
    logger = SimulationLogger(f"sim_contrarian_{timestamp}.log")
    ...
    sample_size = 10 # 50 to 10 for quick test
    ...
    res_new = run_simulation("meta_contrarian", predictor, hr, peds, df_target, returns_dict, race_ids, logger)
    logger.log(f"meta_contrarian: {res_new['recovery_rate']:.1f}% (+¥{res_new['profit']:,})")
    
    # 結果表示
    print("\n" + "="*50)
    print("新戦略 (逆張り) 検証結果 (10%サンプル)")
    print("="*50)
    print(f"{'指標':<15} {'formation_flex':>15} {'meta_contrarian':>15}")
    print("-" * 50)

...
def load_resources():
    ...
    # Random 20 Races (Debug)
    all_race_ids = df_target['race_id'].unique()
    if len(all_race_ids) > 20:
        import random
        random.seed(42)
        selected_race_ids = sorted(random.sample(list(all_race_ids), 20))
        df_target = df_target[df_target['race_id'].isin(selected_race_ids)].copy()
        print(f"DEBUG: Randomly selected 20 races from {len(all_race_ids)}.", flush=True)
                    ...
                    try:
                        ...
                        if recs:
                            for rec in recs:
                                ...
                                print(f"DEBUG BET: {rec['bet_type']} {rec.get('combo_str','?')} Odds:{rec.get('odds',0):.1f} Prob:{rec.get('prob',0):.3f} Div:{rec.get('divergence',0):.2f}", flush=True)
                                
                                pay = verify_hit_logic(race_id, rec, returns)
                                total_results[key]['return'] += pay
                                print(f"DEBUG HIT!: {rec['bet_type']} {rec.get('combo_str','?')} Pay:{pay}", flush=True)
                                if pay > 0:

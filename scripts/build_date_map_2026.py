...
def run():
    ...
    # Filter for 2026
    races_2026 = results[results['race_id'].str.startswith(target_year)]['race_id'].unique()
    print(f"Found {len(races_2026)} races for {target_year} in results.pickle")

    # Filter results to only include target_year
    results = results[results['race_id'].str.startswith(target_year)]
    print(f"Results ({target_year}): {len(results)} rows")
    ...
    hr_clean = hr_clean[hr_clean['date_parsed'].str.contains(target_year)]
    ...
    output_path = os.path.join(os.path.dirname(RAW_DATA_DIR), "date_map_2026.pickle")
    with open(output_path, 'wb') as f:
        pickle.dump(final_map, f)
    print(f"Saved date_map to {output_path}")

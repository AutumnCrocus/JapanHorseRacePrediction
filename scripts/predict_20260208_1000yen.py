    ...
    if horse_results_db is not None:
         ...
         if 'horse_id' in df_processed.columns:
             ...
             if hr_subset is not None and not hr_subset.empty:
                 ...
                 # If filtering fail or empty, pass full db or skip? skipping is safer to avoid crash/slowdown
                 # But might miss features. Pass subset if empty means no history.
                 # Let's assume if subset empty, no history.
                 df_processed = predictor.engineer.add_horse_history_features(df_processed, pd.DataFrame()) # Pass empty to get default cols
...
def main():
    ...
    report_file = f"prediction_report_{date_str}_1000yen.md"
